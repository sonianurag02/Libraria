...
<body>
    ...
    ?>
    <div style="width: 101.29%; height: 50px; margin: -20px;">
        <div style="background-color: #dd8953; margin-left: 20px; padding: 10px; width: 8%; height: 50px; float: left;">
            <h3 style="color: white; margin-top: 0px;"> Trending: </h3>
        </div>
        <div style="background-color: #ffcccc; width: 90.7%; height: 50px; float: left; padding: 10px; ">
            <table>
                <?php
                    while($b2=mysqli_fetch_assoc($b)){
                        echo "<tr style='color: black; width: 400px; margin-top: 5px; float: left;'>";
    ...
    <div class="srch">
        <form action="" class="navbar-form" method="post" name="form1">
            ...
            <button style="background-color: #55cbc3;" type="submit" name="submit1" class="btn btn-default"> Request
            </button>
        </form>
    </div>
    ...
    <?php
        ...
        if(isset($_POST['submit'])){
            ...
            // echo $_POST['department']; //ERROR NOT SOLVED

            $q=mysqli_query($db,"SELECT * from books WHERE name like '%$_POST[search]%' "); // and department='$_POST[department]' {ERROR}
